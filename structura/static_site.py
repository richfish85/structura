...
def _discrepancy_legend() -> str:
    """Shared explanation of machine review levels; levels never promote a record."""
    return (
        "<section><h2>Discrepancy review legend</h2>"
        "<dl>"
        "<dt>L0</dt><dd>No material discrepancy.</dd>"
        "<dt>L1</dt><dd>Precision or channel-detail gap; qualifying event is supported.</dd>"
        "<dt>L2</dt><dd>Plausible retained event; shipment or availability is unknown or forecast-only.</dd>"
        "<dt>L3</dt><dd>Identity, granularity, or cohort-boundary decision remains open.</dd>"
        "<dt>L4</dt><dd>Sources or interpretations materially conflict.</dd>"
        "<dt>OL</dt><dd>Deferred omission lead. This is an audit workflow marker, not a candidate discrepancy level.</dd>"
        "</dl><p>Level describes evidence risk. Disposition records the separate next-action decision.</p></section>"
    )
...
def render_static_site(db_path: Path | str, output_dir: Path | str) -> Path:
    ...
    with connect(db_path, read_only=True) as connection:
        ...
        batches = connection.execute(
            "SELECT b.id, b.batch_key, b.title, b.scope_note, b.workflow_status, COUNT(DISTINCT r.id) AS run_count, COUNT(DISTINCT c.id) AS candidate_count, (SELECT COUNT(*) FROM context_import_runs cr WHERE cr.research_batch_id=b.id) AS context_run_count FROM research_batches b LEFT JOIN research_import_runs r ON r.research_batch_id=b.id LEFT JOIN intake_candidate_records c ON c.import_run_id=r.id GROUP BY b.id ORDER BY b.batch_key"
        ).fetchall()
        batch_rows = []
        for batch in batches:
            path = f"batches/{quote(batch['batch_key'], safe='')}.html"
            batch_rows.append(f"<tr><td><a href=\"{path}\">{html.escape(batch['batch_key'])}</a></td><td>{html.escape(batch['title'])}</td><td>{html.escape(batch['workflow_status'])}</td><td>{batch['run_count']}</td><td>{batch['candidate_count']}</td><td>{batch['context_run_count']}</td></tr>")
            ...
            candidates = connection.execute(
                "SELECT c.*, e.name, e.record_status, e.confidence, COUNT(v.id) AS verification_count, dr.discrepancy_level, dr.disposition AS review_disposition FROM intake_candidate_records c JOIN entities e ON e.id=c.entity_id JOIN research_import_runs r ON r.id=c.import_run_id LEFT JOIN verification_records v ON v.intake_candidate_record_id=c.id LEFT JOIN candidate_discrepancy_reviews dr ON dr.id=(SELECT d2.id FROM candidate_discrepancy_reviews d2 JOIN discrepancy_review_runs rr2 ON rr2.id=d2.discrepancy_review_run_id WHERE d2.intake_candidate_record_id=c.id ORDER BY rr2.imported_at DESC, d2.id DESC LIMIT 1) WHERE r.research_batch_id=? GROUP BY c.id ORDER BY c.candidate_key",
                (batch["id"],),
            ).fetchall()
            ...
            context_runs = connection.execute(
                "SELECT * FROM context_import_runs WHERE research_batch_id=? ORDER BY run_key",
                (batch["id"],),
            ).fetchall()
            review_runs = connection.execute(
                "SELECT * FROM discrepancy_review_runs WHERE research_batch_id=? ORDER BY imported_at, run_key",
                (batch["id"],),
            ).fetchall()
            ...
            candidate_rows = "".join(f"<tr><td><a href=\"../candidates/{quote(candidate['candidate_key'], safe='')}.html\">{html.escape(candidate['candidate_key'])}</a></td><td>{html.escape(candidate['product_name_raw'])}</td><td>{html.escape(candidate['manufacturer_raw'])}</td><td>{html.escape(candidate['record_status'])}</td><td>{candidate['verification_count']}</td><td>{html.escape(candidate['discrepancy_level'] or 'Not reviewed')}<br>{html.escape(candidate['review_disposition'] or '')}</td></tr>" for candidate in candidates) or "<tr><td colspan=\"6\">No imported candidates.</td></tr>"
            audit_rows = "".join(
            ...
            context_run_rows = "".join(
                f"<tr><td><code>{html.escape(row['run_key'])}</code></td><td>{row['intent_count']}</td><td>{row['benchmark_count']}</td><td>{row['source_count']}</td><td><code>{html.escape(row['input_digest'])}</code></td></tr>"
                for row in context_runs
            ) or "<tr><td colspan=\"5\">No purpose or benchmark context imported.</td></tr>"
            review_run_rows = "".join(
                f"<tr><td><code>{html.escape(row['run_key'])}</code></td><td>{row['decision_count']}</td><td><code>{html.escape(row['input_digest'])}</code></td><td>{html.escape(row['importer'])}</td></tr>"
                for row in review_runs
            ) or "<tr><td colspan=\"4\">No discrepancy review decisions imported.</td></tr>"
            body = f"<p><a href=\"../index.html\">Batches</a></p><h2>{html.escape(batch['title'])}</h2>{_discrepancy_legend()}<dl><dt>Batch key</dt><dd><code>{html.escape(batch['batch_key'])}</code></dd><dt>Workflow status</dt><dd>{html.escape(batch['workflow_status'])}</dd><dt>Scope</dt><dd>{html.escape(batch['scope_note'])}</dd></dl><h3>Import runs</h3><table border=\"1\"><thead><tr><th>Run</th><th>Stage</th><th>Records/findings</th><th>Source URLs</th><th>Input digest</th></tr></thead><tbody>{run_rows}</tbody></table><h3>Purpose and benchmark context runs</h3><table border=\"1\"><thead><tr><th>Run</th><th>Purpose claims</th><th>Benchmark observations</th><th>Sources</th><th>Input digest</th></tr></thead><tbody>{context_run_rows}</tbody></table><h3>Discrepancy review runs</h3><table border=\"1\"><thead><tr><th>Run</th><th>Decisions</th><th>Input digest</th><th>Importer</th></tr></thead><tbody>{review_run_rows}</tbody></table><h3>Candidate records</h3><table border=\"1\"><thead><tr><th>Candidate key</th><th>Raw product name</th><th>Raw manufacturer</th><th>Record status</th><th>Verification assertions</th><th>Latest review</th></tr></thead><tbody>{candidate_rows}</tbody></table><h3>Audit findings and omission leads</h3><p>OL is a deferred omission-lead marker. Proposed keys are leads only; they are not candidate entities or discrepancy levels.</p><table border=\"1\"><thead><tr><th>Finding</th><th>Type</th><th>Severity</th><th>Affected/proposed key</th><th>Observation</th><th>Evidence</th><th>Next action</th><th>Disposition</th></tr></thead><tbody>{audit_rows}</tbody></table>"
        ...
        for candidate in candidates:
            ...
            review_decisions = connection.execute(
                "SELECT d.*, rr.run_key, GROUP_CONCAT(t.discrepancy_type, ', ') AS discrepancy_types FROM candidate_discrepancy_reviews d JOIN discrepancy_review_runs rr ON rr.id=d.discrepancy_review_run_id JOIN candidate_discrepancy_review_types t ON t.candidate_discrepancy_review_id=d.id WHERE d.intake_candidate_record_id=? GROUP BY d.id ORDER BY rr.imported_at, d.id",
                (candidate["id"],),
            ).fetchall()
            intent_claims = connection.execute(
                "SELECT i.*, r.run_key, s.title, s.url FROM product_intent_claims i JOIN context_import_runs r ON r.id=i.context_import_run_id JOIN sources s ON s.id=i.source_id WHERE i.intake_candidate_record_id=? ORDER BY CASE i.claim_scope WHEN 'manufacturer_stated' THEN 1 WHEN 'independently_interpreted' THEN 2 ELSE 3 END, r.run_key, i.id",
                (candidate["id"],),
            ).fetchall()
            benchmarks = connection.execute(
                "SELECT o.*, r.run_key, s.title, s.url FROM benchmark_observations o JOIN context_import_runs r ON r.id=o.context_import_run_id JOIN sources s ON s.id=o.source_id WHERE o.intake_candidate_record_id=? ORDER BY o.comparison_group_key, o.benchmark_name, o.metric_name, o.id",
                (candidate["id"],),
            ).fetchall()
            ...
            review_rows = "".join(f"<tr><td><code>{html.escape(row['run_key'])}</code></td><td>{html.escape(row['discrepancy_level'])}</td><td>{html.escape(row['discrepancy_types'])}</td><td>{html.escape(row['disposition'])}</td><td>{html.escape(row['decision_rationale'])}</td><td>{html.escape(row['resolving_evidence'])}</td><td>{html.escape(row['reviewer'])}</td></tr>" for row in review_decisions) or "<tr><td colspan=\"7\">No discrepancy review decision imported.</td></tr>"
            intent_rows = "".join(
                f"<tr><td>{html.escape(row['claim_scope'])}</td><td>{html.escape(row['problem_addressed'])}</td><td>{html.escape(row['target_user'])}</td><td>{html.escape(row['engineering_response'])}</td><td>{html.escape(row['confidence'])}</td><td>{_source_link(row['title'], row['url'])}<br>{html.escape(row['source_locator'] or '')}<br>{html.escape(row['evidence_note'])}</td><td>{html.escape(row['limitations'] or '')}<br>Researcher: {html.escape(row['researcher'])}<br>Run: <code>{html.escape(row['run_key'])}</code></td></tr>"
                for row in intent_claims
            ) or "<tr><td colspan=\"7\">No supported purpose claim has been imported. Absence is not evidence that the product had no purpose.</td></tr>"
            benchmark_rows = "".join(
                f"<tr><td><code>{html.escape(row['comparison_group_key'])}</code></td><td>{html.escape(row['benchmark_name'])}<br>{html.escape(row['metric_name'])}</td><td>{html.escape(str(row['result_value']))} {html.escape(row['result_unit'])}<br>{'Higher is better' if row['higher_is_better'] else 'Lower is better'}</td><td>{html.escape(row['system_configuration'])}<br>Test date: {html.escape(row['test_date'] or '')}</td><td>{html.escape(row['result_provenance'])}<br>Confidence: {html.escape(row['confidence'])}</td><td>{_source_link(row['title'], row['url'])}<br>{html.escape(row['source_locator'] or '')}</td><td>{html.escape(row['limitations'] or '')}<br>Researcher: {html.escape(row['researcher'])}<br>Run: <code>{html.escape(row['run_key'])}</code></td></tr>"
                for row in benchmarks
            ) or "<tr><td colspan=\"7\">No defensibly comparable benchmark observation has been imported.</td></tr>"
            body = f"<p><a href=\"../batches/{quote(candidate['batch_key'], safe='')}.html\">Batch</a> · <a href=\"../runs/{quote(candidate['run_key'], safe='')}.html\">Scout import run</a></p><h2>{html.escape(candidate['product_name_raw'])}</h2>{_discrepancy_legend()}<p>Candidate key: <code>{html.escape(candidate['candidate_key'])}</code><br>Record status: {html.escape(candidate['record_status'])}<br>Confidence: {html.escape(candidate['confidence'])}</p><p>These fields are raw intake. They are not normalized findings.</p><h3>Discrepancy review decisions</h3><p>Each decision is append-only. Level is separate from disposition and never promotes this candidate.</p><table border=\"1\"><thead><tr><th>Review run</th><th>Level</th><th>Types</th><th>Disposition</th><th>Decision rationale</th><th>Evidence that would resolve it</th><th>Reviewer</th></tr></thead><tbody>{review_rows}</tbody></table><h3>Why it existed</h3><p>Purpose claims distinguish manufacturer positioning, independent interpretation, and inference. They do not claim access to a company's private decision-making.</p><table border=\"1\"><thead><tr><th>Claim scope</th><th>Problem addressed</th><th>Target user</th><th>Engineering response</th><th>Confidence</th><th>Evidence</th><th>Limits / provenance</th></tr></thead><tbody>{intent_rows}</tbody></table><h3>What difference it made</h3><p>Only observations sharing a comparison-group key should be compared. Configuration and result provenance remain visible.</p><table border=\"1\"><thead><tr><th>Comparison group</th><th>Benchmark / metric</th><th>Result</th><th>System configuration</th><th>Provenance</th><th>Source</th><th>Limits</th></tr></thead><tbody>{benchmark_rows}</tbody></table><h3>Raw hand-off</h3><table border=\"1\"><tbody>{raw_rows}</tbody></table><h3>Source references</h3><table border=\"1\"><thead><tr><th>Source</th><th>Role</th><th>Note</th></tr></thead><tbody>{evidence_rows}</tbody></table><h3>Verification assertions</h3><p>Assertions below record a verifier's assessment; they do not change the candidate or canonical status.</p><table border=\"1\"><thead><tr><th>Verification run</th><th>Identity</th><th>Manufacturer</th><th>Category</th><th>Release timing</th><th>Confidence</th><th>Evidence</th></tr></thead><tbody>{assertion_rows}</tbody></table>{report_table}<h3>Normalization proposal</h3><p>Proposed labels and hierarchy below do not rewrite the raw record.</p><table border=\"1\"><thead><tr><th>Run</th><th>Display name</th><th>Manufacturer</th><th>Granularity</th><th>Parent</th><th>Category</th><th>Qualifying event</th><th>Decision</th><th>Rationale / questions</th></tr></thead><tbody>{normalization_rows}</tbody></table><h3>Audit findings</h3><table border=\"1\"><thead><tr><th>Run</th><th>Type</th><th>Severity</th><th>Finding</th><th>Evidence</th><th>Next action</th><th>Disposition</th></tr></thead><tbody>{audit_rows}</tbody></table>"
            _write(root, f"candidates/{quote(candidate['candidate_key'], safe='')}.html", _page(candidate["product_name_raw"], body))
    overview_rows = "".join(batch_rows) or "<tr><td colspan=\"6\">No batches.</td></tr>"
    body = f"<h2>Review batches</h2>{_discrepancy_legend()}<p>Entities: {totals['entities']}; sources: {totals['sources']}; import runs are listed by batch below.</p><p>This is a static review export. It contains raw agent hand-offs, not a canonical census.</p><table border=\"1\"><thead><tr><th>Batch key</th><th>Title</th><th>Workflow status</th><th>Runs</th><th>Candidates</th><th>Context runs</th></tr></thead><tbody>{overview_rows}</tbody></table>"
